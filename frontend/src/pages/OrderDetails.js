  ...
  useEffect(() => {
    async function fetchOrder() {
      try {
        setLoading(true);
        setError(null);
        const res = await fetch(`${process.env.REACT_APP_API_URL}/api/payments/order/${id}`);
        if (!res.ok) {
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16 }}>
        <div>
          ...
          <div style={{ marginTop: 6, color: "#374151" }}>
            ...
            {updatedAt && <span style={{ marginLeft: 12 }}><small> • Updated: {formatDate(updatedAt)}</small></span>}
          </div>
        </div>
      ...
      <div ref={printableRef} style={{ marginTop: 20, background: "#fff", padding: 16, borderRadius: 8 }}>
              ...
              return (
                <div key={it._id || i} style={{ display: "flex", gap: 12, padding: 12, borderRadius: 8, border: "1px solid #eef2ff", alignItems: "center" }}>
                  <div style={{ width: 90, height: 90, borderRadius: 8, overflow: "hidden", background: "#f3f4f6", display: "flex", alignItems: "center", justifyContent: "center" }}>
                    {image ? <img src={image} alt={title} style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <div style={{ color: "#9ca3af" }}>No Image</div>}
                  </div>
        ...
        <section style={{ marginTop: 18, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <a
              href={`${process.env.REACT_APP_API_URL}/api/orders/${orderId}/invoice`}
              onClick={(e) => { if (!process.env.REACT_APP_API_URL) e.preventDefault(); }}
              style={{
                textDecoration: "none",
                border: "none",
                background: "#111827",
